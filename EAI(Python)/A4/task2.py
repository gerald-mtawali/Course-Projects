...
Every example of data has a specific input and target output


Play a game against an RPS AGENT
"""
import numpy as np
import random
import csv
...
class RPS_ANN():
    def __init__(self,n_layers, n_hidden, n_input, n_output, learn_rate =0.5):
    ...
    def init_network(self, in_vector, target):
        ...
        for i in xrange(self.n_layers):
            if i == 0:
                ...
                hidd_layer = np.zeros(shape=[self.n_hidden])
    ...
    def init_weights(self):
        ...
        for i in xrange(self.n_layers-1):
            if i == 0:
                weight = np.random.rand(self.n_input, self.n_hidden)
                self.weights.append(weight)
                bias = np.random.rand(self.n_hidden, 1)
                self.biases.append(bias)
            elif i == self.n_layers-2:
                weight = np.random.rand(self.n_hidden, self.n_output)
                bias = np.random.rand(self.n_output,1)                       # the bias value
                self.biases.append(bias)
                self.weights.append(weight)
            else:
                weight = np.random.rand(self.n_hidden,self.n_hidden)
                bias = np.random.rand(self.n_hidden,1)
                self.biases.append(bias)
                self.weights.append(weight)
    ...
    def backprop_error(self, target):
        ...
        idx = 0
        for i in reversed(xrange(self.n_layers-1)):
            ...
            if i == self.n_layers-2:
                ...
                delta_out = np.zeros((self.n_hidden, len(target)))              # HxY
                for j in xrange(self.n_hidden):                                 # the input to the weight(row)
